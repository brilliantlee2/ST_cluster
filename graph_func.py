...
def edgeList2edgeDict(edgeList, nodesize):   #这个是cell-cell矩阵
    graphdict = {}
    tdict = {}
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    sparse_mx = sparse_mx.tocoo().astype(np.float32)                                               #coo格式的稀疏矩阵变成稀疏tensor
...
def preprocess_graph(adj):
    adj = sp.coo_matrix(adj)                 #adj是邻接矩阵
    adj_ = adj + sp.eye(adj.shape[0])        #添加自连接
    rowsum = np.array(adj_.sum(1))           #度矩阵
    degree_mat_inv_sqrt = sp.diags(np.power(rowsum, -0.5).flatten()) #对都矩阵做sys
    adj_normalized = adj_.dot(degree_mat_inv_sqrt).transpose().dot(degree_mat_inv_sqrt).tocoo() #标准化的邻接矩阵 
    return sparse_mx_to_torch_sparse_tensor(adj_normalized) #好像建立矩阵建立的不是拉普拉斯矩阵
